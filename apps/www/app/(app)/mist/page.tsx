...
export default function Home() {
    ...
    return (
        <>
            <section>
                <div className="mx-4 max-w-7xl px-4 py-24 [--color-border:color-mix(in_oklab,var(--color-zinc-200)_75%,transparent)] md:mx-auto dark:[--color-border:color-mix(in_oklab,var(--color-zinc-800)_60%,transparent)]">
...
const MistKitLogo = () => (
    <div
        aria-hidden
        className="bg-linear-to-b rounded-(--radius) inset-ring inset-ring-foreground/15 relative flex size-9 translate-y-0.5 items-center justify-center from-lime-300 to-teal-600 shadow-lg shadow-black/20">
        <BookOpen className="mask-b-from-25% size-6 fill-white/75 stroke-white drop-shadow-sm" />
        <BookOpen className="absolute inset-0 m-auto size-6 fill-white/75 stroke-white opacity-65 drop-shadow-sm" />
    </div>
)
