...
export default function CategoryNotFound({ categoryName, kitName }: { categoryName: string; kitName: string }) {
    ...
    return (
        <section className="bg-muted mb-6 flex min-h-[calc(100dvh-6rem)] flex-col items-center justify-center gap-4 border-b py-16 text-center md:mb-12 dark:bg-transparent">
            <div className="flex gap-4 [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_70%,transparent_100%)]">
                <div className="rounded-(--radius) bg-background size-32 -translate-x-1/3 translate-y-1/2 border shadow-md">
                    <BorderBeam
                        size={100}
                        duration={12}
                        delay={6}
                ...
                <div className="rounded-(--radius) bg-background relative size-32 overflow-hidden border shadow-md">
                    <BorderBeam
                        size={200}
                        duration={12}
                        className="via-foreground/50 from-transparent to-transparent"
                    />
                </div>
            </div>
